...
function TypographyTest(props){
  ...
  React.useEffect(()=>{
    console.log('test_active_style.section', test_active_style.section)
    console.log('test_active_style.title', test_active_style.title)

  },[])

  let [test_active_style, setTestActiveStyle] = React.useState(active_style)
  React.useEffect(()=>{
    console.log('active_style updated', test_active_style.section)
    setTestActiveStyle(active_style)
  }, [active_style])

  return(
    <section className={test_active_style.section} id="typography">
      <h1 className={test_active_style.title}> Typography </h1>
      <hr />
