...
@test "test not existing url" {
  result="$(curl -s -d '{"target_csv": "{ target_csv: 'https://not_exist.hk/opendata/cev/cyberport_event.csv' }"}' -H "Content-Type: application/json" -X POST http://localhost:5001/fir-tryout-f4e7a/us-central1/curl_fetch)"
